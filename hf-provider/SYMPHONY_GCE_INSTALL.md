...
| ------------- |---------------|-------------|
| `HF_DBDIR` | Defined in HostFactory environment as `$HF_DBDIR` | The location where this provider should store its state database
| `HF_TEMPLATES_FILENAME` | `gcpgceinstprov_templates.json` | The name of the templates file
| `GCP_CREDENTIALS_FILE` | Uses Application Default Credentials if not specified | The location of the Google Cloud service account credentials file
| `GCP_PROJECT_ID` | | The ID of the Google Cloud project
| `GCP_INSTANCE_PREFIX` | `sym-` | A string to prepend to all hosts created by this provider
| `PUBSUB_TIMEOUT` | `600` | If the most recent PubSub event was longer ago than this duration, in seconds, the PubSub listener will disconnect. This timeout only applies when the PubSub event listener is automatically launched. Otherwise, the listener will run indefinitely, and the admin should control the lifecycle.
| `PUBSUB_TOPIC` | `hf-gce-vm-events` | The name of the PubSub topic. This variable is for backwards compatibility only.
| `PUBSUB_SUBSCRIPTION` | `hf-gce-vm-events-sub`| The name of the PubSub subscription to monitor for VM events.
| `PUBSUB_LOCKFILE` | `/tmp/sym_hf_gcp_pubsub.lock`  | The name of the file to indicate that the PubSub event listener is active
| `PUBSUB_AUTOLAUNCH` | `true`  | If set to `true`, the provider will attempt to automatically launch the PubSub event listener. If `false`, you will need to launch the PubSub event listener manually, via the command `hf-monitor`. You can launch the daemon inline with a command, with the command `hf-gce <command> --monitor`.
| `AUTO_RUN_TRIM_DB_CMD` | `true`  | Enables the provider to purge the provider database of inactive records. Setting this to `false` would allow for the creation of an batch process to run the trim command external from the provider execution. Make sure to point to the correct configuration file before running the command.
| `RETURNED_VM_TTL` | `30` | Determines how long a returned machine record remains in the database. Value is in days. Example: 30 means any machine older than 30 days will be permanently removed during cleanup.
| `LOGFILE` | A file with a generated name, located in the directory defined by the HostFactory environment variable `$HF_PROVIDER_LOGDIR` | The location of the log file that the provider should log to
| `LOG_LEVEL`  | `WARNING` | Controls the level of log detail that the GCE Provider writes to the log file. Options are `CRITICAL`, `WARNING`, `ERROR`, `INFO`, `DEBUG`.
