...
The RPM package's default `prefix` is `/opt/ibm/spectrumcomputing`.  `dnf install hf-gcpgke-provider.x86_64` will install to the default prefix

To install to an alternative prefix, download the rpm, `dnf download hf-gcpgke-provider`, then use the rpm `--prefix` parameter to choose an alternative prefix `rpm -i --prefix /my/custom/prefix/ *hf-gcpgke-provider*.rpm`
...
The validation script can be found under the [resources/gke_cli/1.2/providerplugins/gcpgke/bin](./resources/gce_cli/1.2/providerplugins/gcpgke/bin) directory. 
