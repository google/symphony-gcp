...
locals {
  ...
  template_fileset = [
    for filename in fileset(local.templates_dir, "*.yaml.tftpl") : "${filename}"
  ]
  templates = [
...
resource "null_resource" "update-configuration-files" {
    ...
    triggers = {
        ...
        logs_ingertor_url = data.terraform_remote_state.orchestrator-base.outputs.bq-ingestor-url
        run_id = local.run_id
    }  
